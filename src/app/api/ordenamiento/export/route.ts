...
export async function GET(req: NextRequest) {
  try {
    ...
    const rows = await getMetricasExportRows("ordenamiento", {
    ...
    const content = buildMetricasExcelContent(rows, "ordenamiento")

    return new NextResponse(content, {
      headers: {
        "Content-Type": "application/vnd.ms-excel; charset=utf-8",
        "Content-Disposition": `attachment; filename="ordenamiento-${today}.xls"`,
      },
    })
  } catch (error) {
    console.error(error)
